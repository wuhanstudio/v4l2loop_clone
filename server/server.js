...
io.on('connection', async (socket) => {
  ...
  socket.on('capture', async () => {
    ...
    fs.stat('../input.jpg', function (err, stats) {

      if (err) {
        return console.error(err);
      }

      fs.unlink('../input.jpg', function (err) {
        if (err) return console.log(err);
      });
    });
    fs.closeSync(fs.openSync('../capture', 'w'));
    getFile('../input.jpg', 100);
  })

  // If user requests a new image
  socket.on('start', async () => {
    console.log('Recording...')
    fs.closeSync(fs.openSync('../start', 'w'));
  ...
  socket.on('stop', async () => {
    ...
    fs.closeSync(fs.openSync('../stop', 'w'));
    const intervalObj = setInterval(function () {

      const fileExists = fs.existsSync('../live.mp4');
      ...
      if (fileExists) {
        ...
        exec('ffmpeg -i ../live.mp4 -vcodec libx264 -acodec aac resources/output.mp4', (err, stdout, stderr) => {
          if (err) {
